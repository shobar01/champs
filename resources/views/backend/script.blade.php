<script>
    ...
    function cekdet(tanggal,kd){
        window.location.href = "{{route('backend')}}?tanggal="+tanggal+"&kdstatus="+kd;
    }
    function cekkd(kd){
        window.location.href = "{{route('backend')}}?kdstatus="+kd;
    }
        ...
        if (result.isConfirmed) {
            // var nmkat = $('#isikatbaru').val();
                ...
                $.ajax({
                    ...
                    data: {"_token": "{{ csrf_token() }}","kdkat":x},
                    dataType: 'json',
                    success:function(data){  
                        if(data['ress'] == true){
                        $('#xx'+x).remove();
    ...
    function adkat(){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{url("simpan_kategori")}}',
                    data: {"_token": "{{ csrf_token() }}","nmkat":nmkat},
                    dataType: 'json',
                    success:function(data){
                        if(data['ress'] == true){
                            $('#barunich').remove();
   ...
   const constraints = {
    ...
    function pilihdepteska(){
        ...
        $.ajax({
            type: 'POST',
            url: '{{url("nipeska")}}',
            data: {"_token": "{{ csrf_token() }}","dept":dept},
            dataType: 'json',
            success:function(data){
                console.log(data)
                for (let i = 0; i < data.length; i++) {
                  ...
                  var foto = data[i]["foto"]=='F'?"{{asset('public/resource/img/champs.png')}}":'https://api2.champ-group.com/champ_dev/asset/champs_api/images_user/'+ data[i]["foto"];
