<script>
    ...
    function cekdet(tanggal,kd){
        window.location.href = "{{route('ticketing')}}?tanggal="+tanggal+"&kdstatus="+kd;
    }
    function cekkd(kd){
        window.location.href = "{{route('ticketing')}}?kdstatus="+kd;
    }
    ...
    function openticket(kdticket){
        ...
        $.ajax({
            type: 'POST',
            url: '{{url("openticket")}}',
            data: {"_token": "{{ csrf_token() }}","kdticket":kdticket},
            dataType: 'json',
                ...
                if(kdstatus == 'T001' || kdstatus == 'T010'){
                    ...
                    if(readby.length > 0){
                        for (let j = 0; j < readby.length; j++) {
                            ...
                            var foto = readby[j]["foto"]=='F'?"{{asset('public/resource/img/user.png')}}":'https://api2.champ-group.com/champ_dev/asset/champs_api/images_user/'+ readby[j]["foto"];
    ...
    function reminder(){
        ...
        if (result.isConfirmed) {
            ...
            var message = $('#txmessage').val();
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{url("reminder")}}?txtujuan='+deptujuan+'&txmessage='+message,
...
{{-- <script>
    ...
    const terimafoto = document.getElementById('terimafoto');
    const terfoto = document.getElementById('terfoto');
    const screenshotImage = document.getElementById('ssimage');
    const buttons = [...controls.querySelectorAll('button')];
    let streamStarted = false;
   ...
   const constraints = {
    ...
    const doScreenshot = () => {
        ...
        document.getElementById('tampilterima').src = canvas3.toDataURL('image/webp');
        screenshotImage.classList.remove('d-none');
        terimafoto.value = canvas3.toDataURL('image/webp');
        terfoto.src = canvas3.toDataURL('image/webp');
        $('#trfototer').modal('hide');
        setTimeout(function(){ 
          canvas3.remove();
          video.pause();
          video.currentTime = 0;
