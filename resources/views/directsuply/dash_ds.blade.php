        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body" style="padding: 0 5px 10px 5px !important;">
                ...
                <div class="fw-container" style="margin-top: 1%;">
                    <div class="fw-body">
                        <div class="content">
                            <form action="{{ route('dashdirecsupply') }}" method="GET" id="form_ds">

                                <input type="hidden" name="kdakses" id="kdakses" value="{{$kdakses}}">
                                <input type="hidden" name="nip" id="nip" value="{{$nip}}">
                                <input type="hidden" name="nipam" id="nipam" value="{{$nip}}">
                                <div class="input-group">
                                    ...
                                    @if($kdakses == 'AVXXX' || $kdakses == 'AVXX3')
                                    <select class="form-control form-control-sm" name="nipam" id="amnips"
                                        ...
                                        @foreach ($am as $sam)
                                        <option value="{{ $sam['nip'] }}" @if ($nip==$sam['nip'] ) selected @endif>
                                            {{ $sam['nmarea'] }} ({{ $sam['nip'] }})
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" name="spin_tglds" id="spin_tgldss" value="{{$ttgl}}"
                                        class="form-control form-control-sm"
                                        style="font-size: 12px; color:black;  font-weight: bold;" max="{{$date_max}}"
                                        min="{{$date_min}}" onchange="pilihtgl()">
                                </div>

                            </form>

                            @if($jsrsltds['lsoutlet']==null)
                            ...
                            <div class="table-responsive" style="overflow-x: hidden">
                                ...
                                <div class="table-responsive" style="overflow-x: hidden; height: 450px; ">
                                    <table id="tbldirects" class="table table-striped table-bordered">
                                        ...
                                        <tbody id="">
                                            @foreach ($jsrsltds['lsoutlet'] as $dfo)
                                            ...
                                            <tr>
                                                ...
                                                <td class="ftsize12 " style=" max-width: 10px">
                                                    <a class='btn btn-sm btn-danger mr-1' title='Ip'><i
                                                            ...
                                                            onclick="btndet('{{ucwords(strtolower($dfo['nmoutlet']))}}#{{$dfo['kdoutlet']}}# {{ucwords(strtolower($dfo['nmsup']))}}#{{$dfo['kdsup']}}')"></i></a>
                                                </td>
...
<script>
    function btndet(isi){  
      ...
      $.ajaxSetup({
        ...
        $.ajax({
            type: 'POST',
            url: '{{url("dfdet_dsuplier")}}',
            data: {"_token": "{{ csrf_token() }}","tanggal":'{{$ttgl}}',"nipam":'{{$nip}}',"kdoutlet":kdotl,"kdsup":kdsup},
            dataType: 'json',
