...
class ReportOrderBrgController extends Controller
{
    ...
    public function repot_ob(Request $request)
    {
        ...
        if ($itype == "btn_excel") {
            ...
            if ($nmdvc == 'Apple') {
                $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])->loadview('rptorderbrg.rpt_download', compact('detail', 'dford', 'itgl', 'kdbrng', 'whdocno', 'tanggalll', 'nmotl', 'nm_lengkap', 'wktorder', 'tglterima', 'ordnip'))->setpaper('a4', 'potrait');
                return $pdf->download('RPT-' . $kdbrng . '.pdf');

                // return view('rptorderbrg.headpdff', compact('detail', 'dford', 'itgl', 'kdbrng', 'whdocno', 'tanggalll', 'nmotl', 'nm_lengkap', 'wktorder', 'tglterima', 'ordnip'));
            } else {
                return view('rptorderbrg.headpdff', compact('detail', 'dford', 'itgl', 'kdbrng', 'whdocno', 'tanggalll', 'nmotl', 'nm_lengkap', 'wktorder', 'tglterima', 'ordnip'));
            }
        } else {
